...
`overrideReasons` | OPTIONAL | *array* of **overrideReasons** | Acknowledgement reasons can be selected by the end user when overriding a card without taking the suggested follow-ups. The CDS service MAY return a list of override reasons to the CDS client. The CDS client MAY present these reasons to the clinician when they dismiss a card. A CDS client MAY augment the override reasons presented to the user with its own reasons.
...
An **overrideReason** is described by the following attributes. No standard set of override reasons exists today; implementers are encouraged to submit suggestions for standardization. 

Field | Optionality | Type | Description
----- | ----- | ----- | --------
`id` | REQUIRED | *string* | An opaque, unique identifier for this override reason. `key` MUST be unique within the card.  To enable analysis of a card's usage, the same key accompanies the same label across CDS Hooks requests for the same CDS client.
