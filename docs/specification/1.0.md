...
`context` | REQUIRED | *object*.  Hook-specific contextual data that the CDS service will need.<br />For example, with the `medication-prescribe` hook this will include a FHIR Bundle of medication orders being prescribed.  For details, see the [Hooks specification](http://cds-hooks.org/hooks/).
