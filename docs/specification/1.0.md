...
`ackReasons` | OPTIONAL | *array* of **ackReasons** | Acknowledgement reasons can be used by the end user when overriding a card without taking the suggested follow-ups. The CDS service MAY return a list of acknowledgement reasons to the CDS client. The CDS client MAY present these reasons to the clinician when they dismiss a card. A CDS client MAY augment the acknowledgement reasons presented to the user with its own reasons.
...
An **ackReason** is described by the following attributes.

Field | Optionality | Type | Description
----- | ----- | ----- | --------
`key` | REQUIRED | *string* | An opaque, unique identifier for this acknowledgement reason. `key` MUST be unique within the card.  To enable analysis of a card's usage, the same key accompanies the same label across CDS Hooks requests for the same CDS client.
`label` | REQUIRED | *string* | A short, human-readable label to display for this acknowledgement (e.g. the CDS Client might render this as the text on a button).
