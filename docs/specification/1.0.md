...
<nobr>`appContext`</nobr> |*string*.  an optional field to share addtional app context from CDS Service to EHR. The `appContext` field should only be valued if the link type is `smart` and is not valid for `absolute` links. The `appContext` field and value will be sent to the SMART app alongside the other launch context when the SMART app is launched.
