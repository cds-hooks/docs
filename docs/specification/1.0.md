...
`preference` | OPTIONAL | *string* | Allows a service to indicate how preferable this suggestion is from among all the available suggestions on the card. Allowed values are: `recommended`, and `alternate`. CDS Hooks clients may choose to influence their UI based on this value, such as pre-selecting certain suggestions etc.
