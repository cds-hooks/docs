...
Field | Optionality | Type | Description
----- | ----- | ----- | --------
`key` | REQUIRED | *string* | An opaque, unique identifier for this acknowledgement reason. `key` MUST be unique for the card.  To enable analysis of a card's usage, the same key accompanies the same label across CDS Hooks requests for the same CDS client.
