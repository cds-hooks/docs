...
`ackReasons` | OPTIONAL | *array* of **ackReasons** | The CDS service MAY return a list of acknowledgement reasons to the CDS client. The CDS client MAY present these reasons to the clinician when they dismiss a card, enabling the clinician to indicate why the guidance was not taken. A CDS client MAY augment the acknowledgement reasons presented to the user with its own reasons.
