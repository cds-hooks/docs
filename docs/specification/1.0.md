...
`overrideReasons` | OPTIONAL | *array* of **OverrideReason** | Acknowledgement reasons can be selected by the end user when overriding a card without taking the suggested follow-ups. The CDS service MAY return a list of override reasons to the CDS client. The CDS client MAY present these reasons to the clinician when they dismiss a card. A CDS client MAY augment the override reasons presented to the user with its own reasons.
...
An **overrideReason** is described by the following attributes. No standard set of **overridereasons** exists today; implementers are encouraged to submit suggestions for standardization. 

Field | Optionality | Type | Description
----- | ----- | ----- | --------
`id` | REQUIRED | *string* | A unique identifier for this **overridereason**. `id` MUST be unique within the card.  To enable analysis of a card's usage, the same key accompanies the same label across CDS Hooks requests for the same CDS client.
...
```json
{
   "ackReasons":[
      {
         "id":"reason-id-provided-by-service",
