...
In order to allow EHRs to discover its services, a CDS service needs to provide a stable endpoint.  A CDS service MAY expose its services at 

```shell
{baseURL}/cds_services  
```
...
Developers of CDS Services MUST provide a stable endpoint for allowing EHRs to discover available CDS Services, including information such as the purpose of the CDS Service, when it should be invoked, and any data that is requested to be prefetched.
