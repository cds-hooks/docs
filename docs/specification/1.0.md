...
In addition to obtaining data through `prefetch`, the CDS service MAY use an access token provided by the EHR to retrieve FHIR resources directly from the EHR's FHIR Server.   
...
The prefetch template is a dictionary of `read` and `search` requests to supply relevant data, where the following variables are defined:
