...
|Variable|Meaning|
---------|--------
|`{{userId}}`|The type and id of the user for this session (e.g. `Practitioner/123`)|

In addition, contextual data appropriate to the hook can be accessed via a token named `context.name-of-the-field`. For example, because the `patient-view` hook defines `patientId` as context, prefetch templates can use the token `context.patientId` as a reference to the id of the patient in context for this activity (e.g. `123`).
