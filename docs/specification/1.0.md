...
An **overrideReason** is described by the following attributes. No standard set of **overridereasons** exists today; implementers are encouraged to submit suggestions for standardization. To enable analysis of a card's usage, the CDS service SHALL send the same `code`, `codesystem`, and `label` across CDS Hooks requests for the same CDS client.

Field | Optionality | Type | Description
----- | ----- | ----- | --------
`code` | REQUIRED | *string* | A code for this **overridereason**. `code` MUST be unique within the card.  To enable analysis of a card's usage, the CDS service SHALL send the same `code` accompanied by the same `label` across CDS Hooks requests for the same CDS client.
...
```json
{
   "ackReasons":[
	 ...
	 "system":"urn:oid:1.2.840.46474.1.3.27.11.6.4.373483.0",
