...
`ruleCode` | OPTIONAL | *[FHIR Coding](http://hl7.org/fhir/datatypes.html#coding)* | A singleton [FHIR Coding](http://hl7.org/fhir/datatypes.html#coding) object that codifies information about the rule used by the CDS Service to generate this card. The codesystem for this code is generally expected to be defined by the CDS Service. This field may optionally be understood by the CDS Client to implement client specific capabilities, such as, preferential treament or deduplication of cards from multiple sources, avoiding repetitive display when the same card is returned across multiple hooks / hook instances (possibly in conjunction with CardID) etc.
