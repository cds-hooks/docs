...
Field | Optionality | Type | Description
----- | ----- | ----- | --------
`code` | REQUIRED | *string* | A code for this **Overridereason**. `code` MUST be unique within the card.  To enable analysis of a card's usage, the CDS service SHALL send the same `code` and 'system' accompanied by the same `label` across CDS Hooks requests for the same CDS client.
