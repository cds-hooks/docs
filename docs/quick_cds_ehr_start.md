...
This tutorial recommends implementing the [security](spec/#security) after succesful open access testing.
...
The attributes available to describe a CDS services is documented in the [CDS hooks specification](spec/#discovery).
...
A non secured FHIR server is important to support testing with a CDS service. When the EHR moves a hook so to production the system to is expected to follow the guidelines in  the [security](spec/#security) requirements.
