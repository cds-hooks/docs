...
Field | Description
----- | -----------
<nobr>`cardInstance`</nobr> |*string*.  an identifier for a particular card. CDS service can respond back with one or more cards which may all point to one or more apps. For example, a service might respond with two cards; each card would be assigned a id `cardInstance`. This allows an EHR to associate a particular CDS hooks request with a particular card, which is necessary to support sending this information back to CDS Service during the app launch if need be.
...
<nobr>`description`</nobr> | *string*. human-readable description of the suggested action. May be presented to the end-user.
